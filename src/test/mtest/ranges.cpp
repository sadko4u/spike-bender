...
MTEST_BEGIN("spike_bender", ranges)
    ...
    MTEST_MAIN
    {
        ...
        MTEST_ASSERT(find_peaks(&ranges, win.channel(0), dspu::db_to_gain(-48.0f), win.length()) == STATUS_OK);
        MTEST_ASSERT(path.fmt("%s/samples/range/02-ranges.csv", resources()) > 0);
        save_ranges(&ranges, &path);
        make_range_mapping(&image, &ranges, win.length());

        // STEP 2: Save produced sample
        MTEST_ASSERT(path.fmt("%s/samples/range/03-ranges.wav", resources()) > 0);
        MTEST_ASSERT(save_audio_file(&image, path.as_string()) == STATUS_OK);

        // STEP 3: Apply gain
        apply_gain(win.channel(0), &ranges, dspu::db_to_gain(-48.0f));
        MTEST_ASSERT(path.fmt("%s/samples/range/04-tuned.wav", resources()) > 0);
        MTEST_ASSERT(save_audio_file(&win, path.as_string()) == STATUS_OK);
    }
