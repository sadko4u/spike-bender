...
MTEST_BEGIN("spike_bender", envelope)

    MTEST_MAIN
    {

        lsp::dspu::Sample in, env, out;
        io::Path path;
        size_t period;
        int file_id = 0;
        ...
        MTEST_ASSERT(path.fmt("%s/samples/envelope/%02d-source.wav", resources(), file_id++) > 0);
        MTEST_ASSERT(save_audio_file(&in, path.as_string()) == STATUS_OK);

        // STEP 1: Estmate RMS values and balance between them
        period      = size_t(dspu::millis_to_samples(SAMPLE_RATE, 40.0f));
        MTEST_ASSERT(spike_bender::estimate_envelope(&env, &in, spike_bender::K_WEIGHT, period) == STATUS_OK);
        MTEST_ASSERT(path.fmt("%s/samples/envelope/%02d-rms-balance.wav", resources(), file_id++) > 0);
        MTEST_ASSERT(save_audio_file(&env, path.as_string()) == STATUS_OK);

//        // STEP 3: Apply RMS balance
//        MTEST_ASSERT(spike_bender::apply_rms_balance(&out, &in, &rmsb) == STATUS_OK);
//        MTEST_ASSERT(path.fmt("%s/samples/envelope/%02d-output.wav", resources(), file_id++) > 0);
